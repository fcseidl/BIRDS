...
class Diffeomorphism:
    ...
    def fwd(self, X):
        ...
        for i in range(X.shape[1]):
            Y[:-1][:, i] = [ X[j][i] + np.polyval(self.polys[j - 1], X[0][i])
    ...
    def rvs(self, Y):
        ...
        for i in range(Y.shape[1]):
            X[0][i] = Y[-1][i]
            X[1:][:, i] = [ Y[j][i] - np.polyval(self.polys[j], Y[-1][i])
...
class DynamicalSystem:
    ...
    def observe(self, N):
        ...
        Returns the data matrix of observed states in the trajectory 
